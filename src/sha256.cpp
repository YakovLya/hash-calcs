...
uint32_t k[64] = { // first 32 bits of cube roots of first 64 primes
...
uint32_t right_rotate(uint32_t a, uint32_t k){ // Like shift (>>) but with cycle for 32bits length
    return (a >> k) | (a << (32 - k));
}
